...
function get_account_form_fields($extra_where = '') {
    ...
    if ($include_admin_use) {
        $fields = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $table_name . " WHERE form_type = %s AND is_active = '1' AND is_register_only_field = '0' AND htmlvar_name != 'uwp_account_password'" . $extra_where . " ORDER BY sort_order ASC", array('account')));
    } else {
        $fields = $wpdb->get_results($wpdb->prepare("SELECT * FROM " . $table_name . " WHERE form_type = %s AND is_active = '1' AND for_admin_use != '1' AND is_register_only_field = '0' AND htmlvar_name != 'uwp_account_password'" . $extra_where . " ORDER BY sort_order ASC", array('account')));
    }
    return $fields;
}

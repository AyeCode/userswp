...
class UsersWP_Form_Builder {
	...
	public function register_fields( $form_type ) {
		...
		$fields     = $wpdb->get_results( $wpdb->prepare( "select field_type, site_title, htmlvar_name, field_icon from " . $table_name . " where form_type = %s and is_register_field=%s order by sort_order asc", array(
			'account',
			'1'
		) ), ARRAY_A );
                            ...
                            <div class="uwp-input-wrap uwp-tab-actions" data-setting="save_button">
								...
								if ( isset( $field_info->htmlvar_name ) && ! in_array( $field_info->htmlvar_name, $default_fields ) ): ?>
                                    <a class="item-delete submitdelete deletion" id="delete-16"
                                       href="javascript:void(0);"
                                       onclick="delete_field('<?php echo esc_attr( $result_str ); ?>', '<?php echo $nonce; ?>')"><?php _e( "Remove", "userswp" ); ?></a>
								<?php endif; ?>
			...
			if ( ! empty( $user_meta_info ) ) {
				...
				$wpdb->update(
					...
					array(
						...
						'site_htmlvar_name' => $site_htmlvar_name,
						'sort_order'        => $field_id,
						'form_id'           => $form_id,
					),
					array( 'id' => $cf )

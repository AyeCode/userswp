...
if ( ! class_exists( 'UsersWP_Settings_User_Sorting', false ) ) {
	...
	class UsersWP_Settings_User_Sorting {
                    ...
                    <ul class="widefat post fixed" style="width:100%;">
                        ...
                        <li>
                            <div class="uwp-input-wrap uwp-tab-actions" data-setting="save_button">
                                ...
                                <a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr($field_id); ?>" href="javascript:void(0);" onclick="delete_field('<?php echo esc_attr($field_id); ?>', '<?php echo wp_create_nonce('uwp_sort_delete_nonce_' . $field_id); ?>', '<?php echo esc_attr($htmlvar_name); ?>', 'user_sorting')"><?php _e("Remove","userswp");?></a>
		...
		public function field_save($request_field = array())
		{
			...
			$check_html_variable = $wpdb->get_var($wpdb->prepare("select COUNT(*) from " . $table_name . " where htmlvar_name = %s",
				array($htmlvar_name)));


			if ( !$id && (int)$check_html_variable > 0) {
				return 'invalid_key';
			}
